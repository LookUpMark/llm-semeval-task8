...
Running large language models on consumer hardware requires aggressive optimization. We employ \textbf{Llama 3.2 3B Instruct} with 4-bit NF4 quantization via bitsandbytes, reducing memory requirements from approximately 6GB to 2GB. This enables the full model to fit on a single GPU while reserving capacity for the retrieval components.
...
\begin{itemize}
    ...
    \item \textbf{LLM}: Llama 3.2 3B with 4-bit NF4 quantization (bitsandbytes)
