...
We implement a \textbf{cyclic agentic graph} using LangGraph~\cite{langgraph}, moving beyond fragile linear pipelines.
...
\begin{enumerate}
    ...
    \item \textbf{Retrieve}: Fetches top-20 candidates from filtered Qdrant vector store.
...
The graph maintains a shared state (`GraphState`) flowing through all nodes, containing conversation history, questions, retrieved documents, and the `fallback\_reason` telemetry. This design decouples the workflow logic from the node implementations.
...
\subsection{Module: Evaluation (eval/evaluate.py)}

We use RAGAS~\cite{ragas} for offline evaluation, employing Llama 3.1 8B as the judge model (no external APIs). The evaluation pipeline:
\begin{enumerate}
    \item Links generated submissions with ground truth from `conversations.json` via `original\_index`.
    \item Computes key metrics:
    \begin{itemize}
        \item \textbf{Faithfulness}: Grounding of answer in context.
        \item \textbf{Answer Relevancy}: Relevance to the query.
        \item \textbf{Context Precision}: Quality of retrieval.
    \end{itemize}
\end{enumerate}
...
We conducted a manual inspection of randomly sampled conversations to contextualize the quantitative metrics. The following examples illustrate the system's behaviors:

\paragraph{Success Case: Multi-Hop Reasoning (ID: govt\_4)}
\textit{User:} ``Are they all by us?'' (referring to Mars missions mentioned in context)\\
\textit{System Answer:} ``No, not all the mentioned missions... joint effort between NASA ... and ESA...''\\
\textit{Analysis:} The system successfully resolved the coreference ``they'' and ``us'' (US/NASA) using the history, retrieved relevant documents about international collaboration, and synthesized a correct answer. This demonstrates the effectiveness of the Rewrite-Retrieve-Generate pipeline.

\paragraph{Safe Failure: Ambiguity Handling (ID: govt\_7)}
\textit{User:} ``Can a 17 year old be the defendant in a claim?''\\
\textit{System Outcome:} \texttt{hallucination\_loop\_exhausted} (Fallback to I\_DONT\_KNOW)\\
\textit{Analysis:} The ground truth answer for this query states: ``It is unclear, however the plaintiff must be at least 18...'' The system, unable to find a definitive ``yes/no'' in the retrieved documents, repeatedly attempted to generate an answer but failed the hallucination check. Rather than fabricating a legal fact, it correctly fell back. This confirms the safety mechanism prevents confidently wrong answers in high-stakes domains.

\paragraph{Retrieval Limit: Short Query (ID: clapnq\_4)}
\textit{User:} ``secondary source''\\
\textit{System Outcome:} \texttt{irrelevant\_docs} (Fallback to I\_DONT\_KNOW)\\
\textit{Analysis:} The user provided a keyword-only query late in the conversation. The Rewriter failed to expand it sufficiently for the dense retriever to find high-score matches, leading the Grader to reject all documents. This highlights a limitation in handling extremely telegraphic user inputs, suggesting potential future work on more aggressive query expansion.
