...
\{s336362, s34520, s336406, s333307, s338009\}@studenti.polito.it}
}
...
We resolve this tension through a \textbf{Parent-Child chunking strategy} implemented via LangChain's \texttt{RecursiveCharacterTextSplitter}. Documents are first split into Parent chunks of \textbf{1200 characters with 100-character overlap}---large enough to contain complete semantic units like full paragraphs or sections while ensuring continuity across chunk boundaries. Each Parent is then subdivided into Child chunks of \textbf{400 characters with 50-character overlap}, optimized for dense vector matching.

The key insight is that we \textit{search over children but generate from parents}. During indexing, each Child chunk stores rich metadata linking it to its parent: \texttt{parent\_text} (the full parent content), \texttt{parent\_title}, \texttt{parent\_url}, and a unique \texttt{parent\_id} (UUID). At query time, the retriever matches against the fine-grained Child embeddings, but the generator receives the broader Parent context extracted from metadata. This achieves the best of both worlds: precise retrieval with coherent generation.

Embeddings are generated using \textbf{BGE-M3} with L2 normalization enabled (\texttt{normalize\_embeddings=True}) and batch size of 16 for memory efficiency. The embedding model runs on CPU to reserve GPU memory for the LLM. All vectors are indexed in Qdrant using \textbf{cosine similarity} as the distance metric.

All documents are indexed in a unified Qdrant collection (\texttt{mtrag\_unified}) with domain metadata attached to each chunk. This enables filtered retrieval: when processing a government conversation, we restrict search to government documents only. Without this filtering, we observed severe cross-domain contamination---a query about NASA's ``Europa Clipper'' retrieved financial documents about ``European markets'' due to semantic similarity in the word ``Europa.'' Indexing is performed in batches of 64 documents for throughput optimization.
...
We use RAGAS for offline evaluation, employing Llama 3.1 8B as the judge model (no external APIs). The evaluation pipeline:
\begin{enumerate}
