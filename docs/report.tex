...
We conducted a manual inspection of randomly sampled conversations to contextualize the quantitative metrics. Table \ref{tab:qualitative} highlights three representative cases:

\begin{table}[htbp]
\caption{Qualitative Analysis of Representative Interactions}
\begin{center}
\begin{tabular}{|p{0.12\linewidth}|p{0.13\linewidth}|p{0.35\linewidth}|p{0.32\linewidth}|}
\hline
\textbf{ID} & \textbf{Type} & \textbf{Interaction} & \textbf{Analysis} \\
\hline
govt\_4 & Success & \textbf{Q}: ``Are they all by us?'' (referenced Mars missions) \newline \textbf{A}: ``No... joint effort between NASA ... and ESA...'' & Resolved coreference correctly using context history; retrieved ESA collaboration info. \\
\hline
govt\_7 & Safe Failure & \textbf{Q}: ``Can a 17 year old be the defendant in a claim?'' \newline \textbf{A}: \texttt{hallucination\_loop\_exhausted} (I\_DONT\_KNOW) & Ground truth states ``It is unclear''. System correctly avoided fabricating a definitive answer. \\
\hline
clapnq\_4 & Retrieval Limit & \textbf{Q}: ``secondary source'' \newline \textbf{A}: \texttt{irrelevant\_docs} (I\_DONT\_KNOW) & Short, keyword-only query. Rewriter/Retriever failed to capture sufficient context. \\
\hline
\end{tabular}
